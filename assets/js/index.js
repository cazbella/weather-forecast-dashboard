...
$("#search-button").on("click", function (event) {
   //prevent default stops browser from clearing info
   event.preventDefault();
   ...
   // Append (prepend to start) the button to the search history container
   $("#history").prepend(historyButton);
...
//copies functiom above but with for loop
function getWeatherForecast(cityName) {
   ...
   fetch(queryURLForecast)
      .then(function (response) {
         ...
         //makes sure only 1 at a time on pagr
         sectionForecast.empty();

         // Create a Bootstrap row
         var forecastRow = $("<div>").addClass("row");
         //method from class activity
         ...
         for (var i = 0; i < data.list.length; i += 8) {
            ...
            var cardColumn = $("<div>").addClass("container fluid col-md-2");
            ...
            // Appends the card to the column
